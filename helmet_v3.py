...
class LcdHandler:
    ...
    def show_countdown(self, seconds):
        ...
        try:
            ...
            self.lcd.show_text(2, 40, 'Helmet v3.0')
...
class CollisionDetector:
    ...
    def _send_sos_alert(self):
        """
        标记 SOS 告警（由检测线程调用，仅设标志位不碰 MQTT）
        MQTT 实际发送由主线程 comm_thread 处理
        """
        ...
        self.sos_sent = True
        self._last_sos_time_ms = time.ticks_ms()
        log('SOS', 'SOS 告警已标记！')

        # 通过共享标志通知主线程发送 SOS（如果有访问引用）
        # 注：detect_thread 中已通过 shared.set_sos() 通知主线程
...
class HelmetSystem:
    ...
    def __init__(self):
        log('SYSTEM', '=' * 50)
        log('SYSTEM', '智能骑行头盔系统 v3.0 启动')
        log('SYSTEM', '=' * 50)

        # ---------- 1. 初始化传感器 ----------
        self.sensor = SensorManager()

        # ---------- 2. 初始化网络和 MQTT（先连 MQTT，避免 AT 命令冲突） ----------
        ...
        if self.mqtt.start_network():
            ...
            for _ in range(10):  # 最多等 10 秒
                if int(time.time()) > 1700000000:  # 时间大于 2023 年即视为已同步
                    ...
                    break
                time.sleep(1)
            # 重试 MQTT 连接（最多 3 次，每次 15 秒超时）
            for attempt in range(1, 4):
                log('MQTT', 'MQTT 连接尝试 {}/3...'.format(attempt))
                ...
                log('MQTT', 'MQTT 连接失败（已重试 3 次），系统将在无 MQTT 模式下运行')
        ...
        self.lcd    = LcdHandler()
        if self.lcd.lcd:
            self.lcd.show_status('Helmet v3.0', 0)
